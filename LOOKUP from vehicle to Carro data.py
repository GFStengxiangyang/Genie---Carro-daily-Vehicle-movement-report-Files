...
#df4['car_plate (eAuto)','Tx Type.1_x','Chassis (eAuto)','Tx Type.1_y'] = df4['car_plate (eAuto)','Tx Type.1_x','Chassis (eAuto)','Tx Type.1_y'].replace(np.nan, 'not in eAuto', regex=True)
df4['car_plate (eAuto)'] = df4['car_plate (eAuto)'].replace(np.nan, 'not in eAuto', regex=True)
df4['Tx Type.1_x'] = df4['Tx Type.1_x'].replace(np.nan, 'not in eAuto', regex=True)
df4['Chassis (eAuto)'] = df4['Chassis (eAuto)'].replace(np.nan, 'not in eAuto', regex=True)
df4['Tx Type.1_y'] = df4['Tx Type.1_y'].replace(np.nan, 'not in eAuto', regex=True)
